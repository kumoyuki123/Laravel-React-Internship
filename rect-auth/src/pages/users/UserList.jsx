  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.response?.data?.errors) {
        ...
        const errorMessage = error.response?.data?.message || 'ユーザーの保存に失敗しました';
    ...
    <Box p={3}>
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ backgroundColor: 'rgba(92,155,16,0.2)' }}>
              ...
              <TableCell><strong>ロール</strong></TableCell>
              <TableCell><strong>作成日</strong></TableCell>
              {canManageUsers && <TableCell><strong>アクション</strong></TableCell>}
            </TableRow>
          </TableHead>
          <TableBody>
                  ...
                  {canManageUsers && (
                    <TableCell>
                      <Box display="flex" gap={1}>
                        <Tooltip 
                          title={canEditUser(user) ? "ユーザー編集" : "編集不可"}
                        >
                          <span>
                            <IconButton
                              size="small"
                        ...
                        <Tooltip 
                          title={canDeleteUser(user) ? "ユーザー削除" : "削除不可"}
                        >
                          <span>
                            <IconButton
                              size="small"
      ...
      <Dialog open={deleteConfirmOpen} onClose={handleDeleteCancel} maxWidth="xs" fullWidth>
        ...
        <DialogContent className='text-center'>
          ユーザー「{userToDelete?.name}」を削除しますか？<br />この操作は取り消せません。
        </DialogContent>
